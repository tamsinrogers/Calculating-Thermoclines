...
# Part 4 - calculate the thermocline depth in Great Pond for July 2019 at 12am
# Run the program from the Terminal by entering "python3 12am.py"

import stats #imports the stats.py program
...
loop.  It then creates a new file, 12am.csv, and writes headers for date and
depth to it.  The if statement in main() extracts each temp value at approximately 12am
...
def main():
    ...
    while len(line) > 0:
        ...
        line = fp.readline()
    thesum = stats.sum(depths)            #sets thesum to the value of the sum() function from stats.py with the data list as its argument
    print("thermocline sum: ", thesum)              #prints the result with the label "sum:"
    themean = stats.mean(depths)          #sets themean to the value of the mean() function from stats.py with the data list as its argument
    print("mean thermocline: ", themean)            #prints the result with the label "mean:"
    themin = stats.min(depths)            #sets themin to the value of the min() function from stats.py with the data list as its argument
    print("min thermocline: ", themin)              #prints the result with the label "min:"
    themax = stats.max(depths)            #sets themax to the value of the max() function from stats.py with the data list as its argument
    print("max thermocline: ", themax)              #prints the result with the label "max:"
    return
